...
static struct ZZbindTBL **ZZbindArr={0}; /* sorted array for pointer conversion */
static int ZZsaSize={0};   /* size of the sorted array */
...
extern "C" {
    int ZZptrSort(const void*,const void*);
}
 ...
 * prototypes if the function is defined above */
    ...
    int ZZbindPtrs(char *p,int type,int size,int vUpd,int val);
    int ZZwrite(FILE *fp,char *buff,int n);
    int ZZread(FILE *fp,char *buff,int n);
    int ZZclose(FILE *fp);
    FILE *ZZopen(char *fileName,char *mode);
    void reduceVClass(int type,int *size);
    void zzdebBlk(char *,void *,int,int);
    void zzdebVObj(int tp,struct ZZstrLST *sL);
#else
    int ZZpOffset();
    int ZZwrite();
    int ZZread();
...
extern int ZZallocPtr;  /* defined in zzincl.h */
...
struct VerCntrl {
    int newType;  /* index of the corresponding new type, -1 for nonexistent */
    int compatible; /* 1=compatible, 0=different pointers */
};
...
struct PtrCntrl {
    int offset;  /* offset in bytes from the beginning of the structure */
    int oSize;   /* object size (combined array&object) in bytes */
};
#define ZZstaticVer VerCntrl
#define ZZstaticPTR PtrCntrl

#ifdef ZZ_ANSI
static int numPtrs(struct ZZstrLST *);
static void virtInit(int);
void ZZsetMask(void);
static void partOf(int par,int chi);
char *ZZbindCnv(char *,int *);
static void ZZwrInher(FILE *);
static void ZZwrPtrs(FILE *);
static char** ZZrdPtrs(FILE*);
static VerCntrl *ZZiCnv(FILE*,int*);
static int ZZbindBind(void);
static PtrCntrl *ptrOffset(struct ZZstrLST *,char**);
static void ZZtmpPerm(struct ZZbindTBL *);
static struct  ZZbindTBL *ZZtmpCnv(FILE*,int,char**,int,int);
...
static void getNewData(FILE *,char**,char**,int);
static int findOffset(int,int,int);
static void oneShuffle(char**,int,int,char*,int);
static void ZZrdInher(FILE*);
static char *ZZselfCnv(int);
static VerCntrl *ZZiCnv(FILE*,int*);
static void createBlock(void);
static void setBindArr(void);
static char *ZZbArray(char*,int*);
static void saveFun(char*,int,int,char*);
static void freeFun(char*,int,int,char*);
...
ZZstaticInt *ptrPtr={(int *)NULL};        /* pointer conversion array */
ZZstaticVer *ic={(VerCntrl *)NULL};               /* type conversion array */
static struct ZZtypeHier *oldInhList={(struct ZZtypeHier *)NULL};
                                                 /* old inheritance hierarchy */
static int *newInh={(int *)NULL}; /* index parallel with oldInhList,
...
ZZstaticPTR *oldOff={(PtrCntrl *)NULL},*newOff={(PtrCntrl *)NULL};
static int nID; /* size of the selfID array */
...
static void delFile();
 ...
 static int varOrg={1}; /* =0 disables variable organization */
...
void ZZsetMask(void)
{
    ZZvoidS *ZZnewObj(int size,int ini);
    void ZZoneMask(char *obj, int type);
#else
...
void ZZsetMask()
{
    ...
    masks=(char **)ZZnewObj(n*(int)sizeof(char *),0);
 ...
 *   Proceed through the object as if array of integers.
 ...
 * the same object. Assume x=max value that can be stored as int
 ...
 * Pointers start on 2B boudnary, but are 4 bytes long. This may lead
 ...
 */
#ifdef ZZ_ANSI
static void virtInit(int tp)
{
#else


static void virtInit(tp)
int tp; /* type index into ZZsList[] to pickup all values */
{
#endif
...
#define STEP int
    ...
    char *p1,*p2;
    int i,n,fsz,csz,prev,df,bypass,justInt,ptrBdry;
    unsigned STEP *r1,*r2;
    unsigned long s1,s2,v1,v2;
    ...
    s1=ZZptrToLong(p1);
    s2=ZZptrToLong(p2);
    if(csz!=fsz && csz!=sizeof(long int)){
...
#ifdef ZZ_ANSI
static int numPtrs(struct ZZstrLST *s)
#else


static int numPtrs(s)
...
/* returns: 0=normal,1=allocation problems,2=file problems,4=strList problems */
{
    ...
    if(ZZpTbType){
        ...
        ptrPtr=(int *)ZZnewObj(numOldPtrs*(int)sizeof(int),0);
...
*/
    ...
    if(ZZinhFLG){
        ...
        ZZdelObj((char *)ZZbindArr); ZZbindArr=NULL; ZZsaSize=0;
...
int par,chi; /* index of the parent and child */
{
#endif
    int kk,oldChTp,newChTp,oldPaTp,newPaTp,oOff,nOff,oldPaSz,newPaSz;
    int i,sz;
    char *p,*s,*m;
 ...
 * Return -1, if match not found. This is an internal error
 * that should never happen.
 *   The child type, cType, may not be known (given -1). In such a case
 * the offset calculation still works, using the offset only.
 */
#ifdef ZZ_ANSI
static int findOffset(int pType,int cType,int oOff)
{
#else


static int findOffset(pType,cType,oOff)
int pType; /* old parent type */
int cType; /* old child type, may be -1 if not known */
int oOff;  /* old offset calculated from pointers */
/* returns new offset, -1 if not found */
{
#endif
    int i,k1,k2,k,oSt,nSt,nOff,nTp,oTp;
...
static void setNewInh()
{
    ...
    if(i>0){
        /* allocate the array */
        newInh=(int *)ZZnewObj((i+1)*(int)sizeof(int),0);
...
void ZZafterOpen()
{
    ...
    if(ZZbindArr)ZZdelObj((char*)ZZbindArr);
    ZZbindArr=NULL;
 ...
 * - first character a,A,g=autom.pointer,c=char,i=int,f=float,s=(char*)
...
char **p; /* corresponding pointer list */
{
    ZZvoidS *ZZnewObj();
#endif
    int i,i1,i2,k,kk,kkk,n,sz;
    char c,*pp;
    PtrCntrl *r;

    n=numPtrs(s);
    r=(PtrCntrl *)ZZnewObj(n*(int)sizeof(struct PtrCntrl),0);
    ...
    for(k=0;s[k].numPtr>=0;k++){
        ...
        for(i=i1, n=s[k].start; i<i2; i++, n+=kk){
            ...
            c=pp[0];
            if(c=='a'||c=='A'||c=='g'||c=='s')kk=(int)sizeof(char *);
            else if(c=='i')kk=(int)sizeof(int);
            else if(c=='c')kk=1;
            else if(c=='f')kk=(int)sizeof(float);
            else {ZZmsgS(29,p[i]); kk=(int)sizeof(char *);}
            kk=kk*sz;
            r[i].oSize=kk;
...
/* Shuffle pointers inside objects, if the organization changed */
#ifdef ZZ_ANSI
static void oneShuffle(char **newp,int k1,int n,char *fObj,int nSize)
{
    ZZvoidS *ZZnewObj(int size,int ini);
...
int k1; /* old type index */
int n;  /* array size */
char *fObj; /* future object, if NULL, allocate a new one */
int nSize; /* new larger size; =0 means use normal size */
{
    ZZvoidS *ZZnewObj();
    void ZZstTime();
    int ZZdelObj();
#endif
    int k2,k,kk,kp,ks,i,pz,sz,pn,ps,ss,pi,si;
...
FILE *fp; /* file to read from - ASCII */
{
    ...
    if(ZZpTbType){
        /* allocate the array */
        n=numPtrs(oldTypes);
        pp=(char **)ZZnewObj(n*(int)sizeof(char *),0);
        ...
        for(i=0;i<n;i++){
            if(ZZbindASCII){
                ...
                ZZread(fp,(char *)(&b),(int)sizeof(struct ZZbinStore),"ZZbinStore");
                ZZread(fp,nn,b.size,"old pointers");
            }
            pp[i]=(char *)ZZnewObj((int)strlen(nn)+1,0);
...
FILE *fp; /* file to read from - ASCII */
{
    ...
    if(i>0){
        ...
        oldInhList= (struct ZZtypeHier *)ZZnewObj(
                  (i+1)*(int)sizeof(struct ZZtypeHier),0);
        ...
        for(i=0;;i++){
            if(ZZbindASCII){
                ...
                ZZread(fp,(char *)(&b),(int)sizeof(struct ZZbinStore));
                ZZread(fp,(char *)(&(oldInhList[i])),b.size);
                ZZread(fp,(char *)(&b),(int)sizeof(struct ZZbinStore));
                ZZread(fp,name,b.size);
            }
            oldInhList[i].name=(char *)ZZnewObj((int)strlen(name)+1,0);
    ...
    if(ZZpTbType){
        ...
        for(i=0;i<n;i++){
            if(ZZbindASCII)fprintf(fp,"%s\n",ZZpList[i]);
                ...
                b.ptr=ZZpList[i];
                b.strInd=(-1);
                b.size=(int)strlen(ZZpList[i])+1;
                ZZwrite(fp,(char *)(&b),(int)sizeof(struct ZZbinStore));
                ZZwrite(fp,ZZpList[i],b.size);
            }
        }
    }
    return;
        ...
        for(i=0;;i++){
            if(ZZbindASCII)fprintf(fp,"%d %d %d %s\n",
              ZZiList[i].type,ZZiList[i].offset,ZZiList[i].num,ZZiList[i].name);
                ...
                b.strInd=(-1);
                b.size=(int)sizeof(struct ZZtypeHier);
                ZZwrite(fp,(char *)(&b),(int)sizeof(struct ZZbinStore));
                ZZwrite(fp,b.ptr,b.size);
                b.ptr=ZZiList[i].name;
                b.strInd=(-1);
                b.size=(int)strlen(ZZiList[i].name)+1;
                ZZwrite(fp,(char *)(&b),(int)sizeof(struct ZZbinStore));
                ZZwrite(fp,ZZiList[i].name,b.size);
            }
...
#ifdef ZZ_ANSI
int ZZmainEntry(struct ZZbindTBL *start,char **pp,char **type,int n)
{
#else


int ZZmainEntry(start,pp,type,n)
...
int n;        /* given size of pp[] and type[] */
{
#endif
    struct ZZbindTBL *tc,*tp;
    int i,j,k,err,arrFlg;
 ...
 */
#ifdef ZZ_ANSI
int ZZbindPtrs(char *obj,int newTp,int newSize,int vUpd,int val)
{
    void ZZsetVirt(char *obj, char **mast,int type,int blst);
    void ZZmarkPtr(char **,int);
#else


int ZZbindPtrs(obj,newTp,newSize,vUpd,val)
...
int val;   /* value to mark bitmap, 0 or 1 */
/* returns number of binding errors */
{
    ...
    void ZZmarkPtr();
#endif
    int i,ii,k,k1,k2,err,state,strt,sz,msz;
    char c,*p,*s,**pp,*ss;
	...
	strt=ZZsList[newTp].start;
	    ...
	    for(i=0;i<ii;i++,pp++){
	        if(i==ZZsList[newTp].selfIndex){
                    ...
                    else (*pp)=ZZselfCnv(newTp);
                }
	        else {
                   state=err; ss= *pp;
                   if(ZZmemSwitch==2)ZZmarkPtr(pp,val);
                   else  (*pp)=ZZbindCnv(*pp,&err);
	...
	prp=(struct ZZproperty*)obj;
        if(ZZmemSwitch==2)ZZmarkPtr(&(prp->codeName),val);
	else prp->codeName=ZZbindCnv(prp->codeName,&err);
        if(ZZmemSwitch==2)ZZmarkPtr((char **)(&(prp->next)),val);
	else prp->next=(struct ZZproperty*)ZZbindCnv((char*)(prp->next),&err);
	c=prp->codeName[0];
	if(c!='i' && c!='f' && c!='c'){
            if(ZZmemSwitch==2)ZZmarkPtr((char **)(&(prp->propType.arrayProp)),val);
            else prp->propType.arrayProp=(struct ZZaProp*)ZZbindCnv(
	...
	else pap->array=ZZbindCnv(pap->array,&err);
    }
    else if(newTp== -4){            /* array header */
        aa=(ZZarray*)obj;
        if(ZZmemSwitch==2)ZZmarkPtr(&(aa->array),val);
        else aa->array=ZZbindCnv(aa->array,&err);
    }
    else if(newTp== -5){  /* char* pointer, or an array of these */
        msz=newSize/(int)sizeof(char *);  /* size of the whole block */
        for(pp=(char**)obj, i=0; i<msz; pp++, i++){
            if(ZZmemSwitch==2)ZZmarkPtr(pp,val);
	    else (*pp)=ZZbindCnv(*pp,&err);
        }
    }
    /* -1=plain block, -5 char*, -6=int, -7=float: do nothing */
...
{
#endif
    int j,err,oldTp,newTp,arrSize,good;
    struct ZZbindTBL *tp;
    ...
    for(j=0;j<ZZbindSize;j++){
        for(tp=ZZbindTable[j];tp;tp=tp->next){
            ...
            if(oldTp>=0){
                ...
                if(newTp<0){
                    ZZmsgS(11,"not eliminating old type object"); /* inter.err*/
                    err++;
                    good=0;
                }
 ...
 * new environment */
#ifdef ZZ_ANSI
static char *ZZselfCnv(int ind)
#else


static char *ZZselfCnv(ind)
int ind; /* given index into ZZsList[] */
#endif
...
/* ---------------------------------------------------------------- */
/* Use the internal list of objects, and hook it up into a hash table.
...
{
    ZZvoidS *ZZnewObj(int size,int ini);
    int ZZhashPtr(char *key,int size),ZZdelObj(char *p);
#else
...
#ifdef DOS
    if(ii>0x1FFF)ii=0x1FFF; /* array size max 2B integer */
#endif
    ZZbindSize=(int)ii;
    
    /* allocate the hash table */
    ZZbindTable=(struct ZZbindTBL**)ZZnewObj(ZZbindSize*(int)sizeof(char *),1);
    ...
    for(tc=start;tc;tc=tcc){
        ...
        if(tc->strIndex<0 || ic[tc->strIndex].newType>=0){ 
            ...
            for(tp=ZZbindTable[i];tp;tp=tp->next){
                /* if the same size, use external(older)type, but internal data */
...
#ifdef SORT_C_LINKAGE
extern "C" {
int ZZptrSort(const void *p1,const void *p2)
{
    ZZptrIntType i1,i2;
 ...
 */
#ifdef ZZ_ANSI
static VerCntrl *ZZiCnv(FILE *fp,int *n)
{
    int ZZdelObj(char *p);
    ZZvoidS *ZZnewObj(int size,int ini);
#else
...
/* returns pointer to the beginning of the index array, NULL if error */
{
    ...
    for((*n)=0;;(*n)++){ /* count number of old types, as you read */
        ...
        if(ZZbindASCII){
            ...
            else if(ZZnTbType==8)bb=sSize-2*(int)sizeof(char *);
            else if(ZZnTbType==7)bb=sSize-3*(int)sizeof(char *);
            else ZZmsgDD(5,ZZnTbType,ZZpTbType);

            r=ZZread(fp,(char *)(str),bb);
            if(bb!=r){
            ...
            r=ZZread(fp,nn,nSize);
            if(nSize!=r)ZZmsgD(13,r); /* wrong records on the bind file */
        }
        np+=str->numPtr;
        str->strName=(char *)ZZnewObj((int)strlen(nn)+1,0);
        if(!(str->strName)){
	...
	if(str->numPtr<0)break;
        ...
        space=str->start + ((str->numPtr) * (int)sizeof(char *));
    ...
    if(str->start==0){ /* flags data from a big memory block */
        if(ZZbindASCII){
            ...
            r=ZZread(fp,(char *)(&(bBlock)),(int)sizeof(struct bigBlock));
            if(r!=(int)sizeof(struct bigBlock))ZZmsgD(13,r);
        }
    ...
    oldTypes=(struct ZZstrLST*)ZZnewObj(((*n)+1)*(int)sizeof(struct ZZstrLST),0);
    ic=(VerCntrl*)ZZnewObj(((*n)+1)*(int)sizeof(struct VerCntrl),0);
    if(!oldTypes || !ic){
 ...
 */
#ifdef ZZ_ANSI
static struct ZZbindTBL *ZZtmpCnv(FILE *fp,int n,char **oldPtrList,
int allCompat,int numKeys)
{
    void ZZgetPtrs(FILE *fp,char *p,struct ZZstrLST *s,int t,char **pList);
    int ZZioFunction(FILE *fp,int mode,int type,char *p);
    int ZZfindSize(struct ZZstrLST *,int);
    ZZvoidS *ZZnewObj(int size,int ini);
#else
...
int n;    /* size of table ic[] */
char **oldPtrList; /* list of old pointer names */
int allCompat; /* 1 if all types are compatible, 0 otherwise */
...
/* returns start of the temp.conversion table, NULL if problems */
{
    ...
    int lastType;       /* type of the last object */
    int justUpdate; /* =1 merge into existing object possible, =0 otherwise */

    int nk,kk,num,bSize,size,type,k,err,ts,cpTp,fSize,fType,delType,ki,nfs;
    int newSize,nSize;
    char *newp,*p,*pp,*fObj;
    ...
    x1=x2=0;
    lastObj=NULL;
    bSize=(int)sizeof(struct ZZbinStore);
    sc=last=NULL;
    for(nk=0;;nk++){ /* loop ends on negative size header */
        ...
        if(ZZbindASCII){
            ...
            if(bs.strInd==(-5))kk=num*(int)sizeof(char *);
            
            if(!justUpdate){
#ifdef LARGE_POINTER
                if(ZZmemSwitch==1)newp=(char *)((long)(bs.ptr)+bBlock.offset);
#else
            ...
            bs.size=kk; /* object treated as having this possibly new size */
            ...
            for(k=kk=0, p=newp, pp=fObj; k<num; k++, p+=size){
                ...
                if(delType){
                    ...
                    if(ZZioFunction(fp,0,fType,p)){ kk=1; break; }
                }
                else {
                    if(oldTypes[type].numPtr>0) /* avoid when no pointers */
                        ZZgetPtrs(fp,p,oldTypes,type,oldPtrList);
                    if(ZZioFunction(fp,2,fType,pp)){ kk=1; break; }
                }
                if(pp)pp+=fSize;
            }
            if(kk){
                ZZmsgSD(12,p,type); /* error in ASCII format when readBind() */
                err++;
                break;
            }
            if(!cpTp){
            ...
            if(ZZread(fp,newp,bs.size)!=bs.size){
        ...
        if(bs.strInd==(-4) && ic){
            ...
            if(ap->hshType>=0)ap->hshType=ic[ap->hshType].newType;
        }
        ...
        if(!justUpdate){
	    ts=(int)sizeof(struct ZZbindTBL);
 ...
 */
#ifdef ZZ_ANSI
static void getNewData(FILE *fp,char **pp,char **tp,int n)
{
    int ZZioFunction(FILE *fp,int mode,int type,char *p);
    int ZZfindSize(struct ZZstrLST *,int);
    ZZvoidS *ZZnewObj(int size,int ini);
...
int n;    /* size of table ic[] */
{
    int  ZZioFunction(), ZZfindSize();
    ZZvoidS *ZZnewObj();
#endif
    struct ZZbinStore bs;
    int i,k,kk,size,type,fType,num,err,bSize;
    char *newp,*p;
    ...
    for(i=0;;){ /* i counts key entries, loop ends on negative size header */
        ...
        /* convert all internal pointers to the new location */
 ...
 */
#ifdef ZZ_ANSI
void ZZsetVirt(char *obj,char **mst,int type,int val)
{
    void ZZmarkPtr(char **,int);
    void ZZvSet(int);
#else
...
int type;  /* type index */
int val; /* 0 or 1, value to mark bitmap, only when ZZmemSwitch=2 */
{
    void ZZmarkPtr();
    void ZZvSet();
#endif
...
#define NEAT_TRICK
...
#ifdef NEAT_TRICK
    ZZvirtCntr=obj;
    if(type>=0)ZZvSet(type); /* sets vf. and vc. pointers */
#else
    ...
    /* this loop works only when sizeof(char *)==sizeof(int) */
 ...
 */
#ifdef ZZ_ANSI
void ZZoneMask(char *m,int type)
{
#else
...
/* ---------------------------------------------------------------- */
/* Pointer conversion using binary search.
 * This function handles some special situations that cannot be converted
 * with the hash table:
 * - Pointer which leads into an existing array must be replaced by
 ...
 */
#ifdef ZZ_ANSI
static char *ZZbArray(char *p,int *err)
{
#else
...
int *err;  /* add 1 to the count, if error encountered */
/* returns the new pointer, NULL if error */
{
#endif
    int i1,i2,ind,k,oOff,nOff,oSz,nSz,oTp,nTp;
    char *ret;
    ZZptrIntType pk,pp;

    if(!p || !ZZbindArr)return(NULL); /* just an additional protection */
...
/* ---------------------------------------------------------------- */
/* setup an array sorted by the old starting address.
...
#ifndef GNU
#ifndef SUN3_0
  void qsort(void *,size_t,size_t,int (*cmpF)(const void *,const void *));
#endif
#endif
#endif
#endif
...
static void setBindArr()
{
    ...
    for(i=n=0;i<ZZbindSize; i++){
        for(tc=ZZbindTable[i];tc;tc=tc->next)n++;
    }
    ZZsaSize=n;

    ZZbindArr=(struct ZZbindTBL **)ZZnewObj(ZZsaSize*(int)sizeof(char *),0);
    if(!ZZbindArr){ZZmsgS(1,"setBindArr()"); return;}

    /* traverse the hash table, and load all objects into the array */
    for(i=0,n=0;i<ZZbindSize; i++){
        for(tc=ZZbindTable[i];tc;tc=tc->next){
            ZZbindArr[n]=tc;
            n++;
        }
    }
    qsort((void *)ZZbindArr,ZZsaSize,(int)sizeof(char *),ZZptrSort);
}
/* ---------------------------------------------------------------- */
 ...
 */
#ifdef ZZ_ANSI
char *ZZbindCnv(char *p,int *err)
{
    int ZZhashPtr(char *p,int size); 
#else


char *ZZbindCnv(p,err)
char *p;   /* old pointer to be found */
int *err;  /* error counter, add 1 in case of conversion error */
/* returns the new pointer, NULL if unsuccessful */
{
    ...
    if(p){
#ifdef LARGE_POINTER
        if(ZZmemSwitch==1)return((char *)((long)p+bBlock.offset));
#else
        ...
        if(!ret){
            if(!ZZbindArr)setBindArr(); /* first time, setup ZZbindArr */
...
/* ---------------------------------------------------------------- */
#ifdef ZZ_ANSI
void ZZwriteASCII(int strSize,int typeInd,char *fileName,char *pp,
                  int num,int key)
{
    int ZZioFunction(FILE *fp,int mode,int type,char *p);
...
int strSize;        /* structure size in bytes */
int typeInd;        /* index of the type, -1 for char string or binary block */
char *fileName;     /* file name for output */
char *pp; /* pointer to the block to be saved */
int num; /* number of elements in the array to be saved */
int key; /* key=1 when key entry, key=0 for writing out an object,
            key= -1 for a sub-object */
{
...
#endif
    int i,ii,j,k,kk,m,mm,m1,m2,pSize,it,sz;
    char *t,**tt,*inj;
    struct ZZstrLST s;
    struct ZZproperty **r;
    ZZarray *pArr;
    ...
    static int level={0}; /* keeps level of the object, 1=parent */

    if(pp==NULL)return;
    level++;

    /* derive strSize if a character string */
    if(typeInd== -1 && num==0){num=1; strSize=(int)strlen(pp)+1;}
    pSize=(int)sizeof(char *);
    ...
    if(ZZbindFile){
        ...
        for(i=0, t=pp;i<num;i++, t+=strSize){
            ZZioFunction(ZZbindFile,1,typeInd,t);
        }
	        ...
	        for(k=0,t=pp;k<num;k++,t+=strSize){
                    ...
                    if(m1>=0){
                        for(i=m1, tt=(char**)(t+mm+m1*pSize); i<m2; i++, tt++){
                         ZZwriteASCII((int)sizeof(ZZarray),-4,fileName,*tt,1,-1);
                        }
                    }
                }
	    }
...
int type; /* type to recursively process */
int *size; /* size to reduce */
#endif
{
    int n1,n2,n,tp;
    char *nm;
...
#ifdef ZZ_ANSI
typedef void (*SAFUN)(char *ptr,int type,int size,char *priv);
int ZZsaveAll(char *file)
{
    void ZZswpFun(SAFUN saveFun,char *file);
#else
...
#ifdef ZZ_ANSI
typedef void (*FRFUN)(char *ptr,int type,int size,char *priv);
int ZZfreeAll(void)
{
    void ZZswpFun(FRFUN freeFun,char *file);
#else
...
/* ----------------------------------------------------------------------- */
#ifdef ZZ_ANSI
static void saveFun(char *ptr,int type,int size,char *priv)
{
    void ZZwriteBind(int size,int type,char *priv,char *ptr,int num,int key);
#else


static void saveFun(ptr,type,size,priv)
char *ptr;  /* pointer to the object */
int type;   /* name of the structure */
...
#ifdef ZZ_ANSI
static void freeFun(char *ptr,int type,int size,char *priv)
{
    int ZZdelObj(char *p);
#else
...
#ifdef ZZ_ANSI
int ZZsaveKeys(char *fileName,struct ZZstrLST *sL,char **v,char **t,int n)
{
    void ZZwriteBind(int size,int type,char *priv,char *ptr,int num,int key);
#else


int ZZsaveKeys(fileName,sL,v,t,n)
char *fileName;
...
#ifdef ZZ_ANSI
void ZZcloseBind(char * fileName)
{
#else
...
char *fileName;
{
    ...
    if(ZZbindASCII){
        ...
        ZZwrite(ZZbindFile,(char *)(&b),(int)sizeof(struct ZZbinStore));
...
/* --------------------------------------------------------------------- */
/* add a new output file to the internal list of currently open files */
/* The new file pointer/handle is supposed to be already in ZZbindFile */
#ifdef ZZ_ANSI
static void setFile(char *fileName)
{
    ZZvoidS *ZZnewObj(int size,int ini);
#else
...
char *fileName;
{
    ...
    fi=(FileInfo *)ZZnewObj((int)sizeof(struct FileInfo),0);
...
/* --------------------------------------------------------------------- */
/* delete a file from the internal list of currently open files */
#ifdef ZZ_ANSI
 ...
 * longer pointers (for example from the DOS medium memory model to UNIX
 ...
 * from oldOff[] */
{
    ...
    for(i=i1; i<i2; i++){
        ...
        for(k=0;k<kk;k+=kd, b+=kd){
            ...
            if(c[0]=='i')      {sscanf(a,"%d",&(u.i)); kd=(int)sizeof(int);}
            else if(c[0]=='f') {sscanf(a,"%f",&(u.f)); kd=(int)sizeof(float);}
            else if(c[0]=='c') {sscanf(a,"%d",&x); u.c=x; kd=(int)sizeof(char);}
#ifdef LARGE_POINTER
            else               {sscanf(a,"%lu",&(u.p)); kd=(int)sizeof(char *);}
#else
            else               {sscanf(a,"%u",&(u.p)); kd=(int)sizeof(char *);}
#endif
 ...
 */
#ifdef ZZ_ANSI
void ZZwriteBind(int strSize,int typeInd,char *fileName,char *pp,int num,int key)
{
...
void ZZwriteBind(strSize,typeInd,fileName,pp,num,key)
int strSize;        /* structure size in bytes */
int typeInd;        /* type index, -1 for a char string or binary block */
char *fileName;     /* file name for output */
char *pp; /* pointer to the block to be saved */
int num; /* number of elements in the array to be saved */
int key; /* key=1 for key entry, key=0 for writing out regular object */
{
    void ZZwriteProp(),ZZwriteBind(),*ZZnewObj();
#endif
    int i,ii,j,k,n,nn,m,mm,m1,m2,pSize;
    char *t,**tt;
    struct ZZproperty **r;
    struct ZZbinStore b;
    struct ZZstrLST s;
    ...
    if(!getFile(fileName)){ /* first operation on this file */
        ...
        if(ZZbindFile){
            ...
            for(i=0;;i++){
                ...
                ii=(int)strlen(ZZsList[i].strName)+1;
                s.strName=ZZintToPtr(ii);
                n=(int)sizeof(struct ZZstrLST);

                if(ZZnTbType==9)nn=n;
                else if(ZZnTbType==8)nn=n-(int)sizeof(char *);
                else if(ZZnTbType==7)nn=n-2*(int)sizeof(char *);
                else ZZmsgDD(5,ZZnTbType,ZZpTbType);

                /* record if the big memory block is being used */
	        if(s.numPtr<0 && ZZmemSwitch && ZZmemStart)s.start=0;

                ZZwrite(ZZbindFile,(char *)(&s),nn);
                ZZwrite(ZZbindFile,ZZsList[i].strName,ii);
	        if(ZZsList[i].numPtr<0)break;
	    }
            if(ZZmemSwitch && ZZmemStart){
                ...
                bBlock.offset=0;
                ZZwrite(ZZbindFile,(char *)(&bBlock),(int)sizeof(struct bigBlock));
            }
            if(ZZpTbType)ZZwrPtrs(ZZbindFile);
            ZZwrInher(ZZbindFile); /* decides by itself whether to print */
        }
    ...
    if(ZZbindFile){
        ...
        b.strInd=typeInd;
        b.size=num*strSize;
        n=(int)sizeof(struct ZZbinStore);
        ZZwrite(ZZbindFile,(char *)(&b),n);
        ZZwrite(ZZbindFile,pp,b.size);
        if(key)return;
	    ...
	    mm=ZZsList[typeInd].start;
	        ...
	        for(k=0,t=pp;k<num;k++,t+=strSize){
                    ...
                    if(m1>=0){
                        for(i=m1, tt=(char**)(t+mm+m1*pSize); i<m2; i++, tt++){
                          ZZwriteBind((int)sizeof(struct ZZarray),-4,fileName,*tt,1,key);
                        }
                    }
                }
	    }
...
#ifdef ZZ_ANSI
void zzdebHex(char *ms,char *bf,int n)
#else
...
#ifdef ZZ_ANSI
void zzdebPtrs(char *p,int i,struct ZZstrLST *types)
#else


void zzdebPtrs(p,i,types)
char *p;  /* pointer to the object */
int i;    /* index into the type table, -1 means no pointers */
...
/* ----------------------------------------------------------------------- */
#ifdef ZZ_ANSI
void zzdebBlk(char *label, void *obj, int size, int type)
#else
void zzdebBlk(label,obj,size,type)
char* label;
void *obj;
int size;
int type; /* given only as info to display, any value will work */
#endif
{
    int sp,si,i;
    short *ip;

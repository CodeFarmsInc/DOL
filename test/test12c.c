/* Test of DOUBLE_TREE:
 * Exactly the same problem as test12a.c, except that the saving
 * is done on an ASCII file.
 */
...
#include "zzfunc.c"
#ifdef ZZ_ANSI
int zz_out_Node(FILE *fp,Node *p){
    fprintf(fp,"%d\n",p->width);
    return(0);
}
int zz_inp_Node(FILE *fp,Node *p){
    if(!fgets(buff,BSIZE,fp))return(1);
    sscanf(buff,"%d",&(p->width));
    return(0);
}
#else
int zz_out_Node(fp,p) FILE *fp; Node *p; {
    fprintf(fp,"%d\n",p->width);
    return(0);
}
int zz_inp_Node(fp,p) FILE *fp; Node *p; {
    if(!fgets(buff,BSIZE,fp))return(1);
    sscanf(buff,"%d",&(p->width));
    return(0);
}

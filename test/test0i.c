/* The same netlist example as in test0a - test0h, except that
 * a COLLECT is used instead of a TRIANGLE. This means that
 * we cannot traverse from a pin back to Block or Net.
 * The output is reduced, because the pin-parent information is not
 * available.
 * The program also tests other COLLECT commands:
 * ZZ_MERGE, ZZ_FORWARD, ZZ_CHILD, ZZ_DELETE, ZZ_SET, and ZZ_SORT
 *
 * Zortech 2.0 compiler runs out of memory on this simple example
 * for the large memory model (medium memory model runs fine).
 */
...
ZZ_ORG_SINGLE_COLLECT(byBlock,Block,Pin);
ZZ_ORG_SINGLE_COLLECT(byNet,Net,Pin);
...
#endif
{
    ...
    Pin *pp,*pr;
#ifdef ZZ_ANSI
    int nSort(const void *p1,const void *p2);
...
/* ------------------------------------------------------------- */
#ifdef ZZ_ANSI
int nSort(const void *s1,const void *s2)
#else
int nSort(s1,s2)
void *s1,*s2;
#endif
...
/* ------------------------------------------------------------- */
#ifdef ZZ_ANSI
int pSort(const void *s1,const void *s2)
#else
int pSort(s1,s2)
void *s1,*s2;
#endif

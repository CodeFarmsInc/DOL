/* Test of DOUBLE_TREE:
 *
 * Interactive tree manipulation similar to the NIAL language.
 * Each node of the tree is a two-dimensional table of nodes.
 * A simple node is a special case of 1*1 table.
 * With each node, we associated a name and an atom, which
 * can have type int, float, or char*.
 * The menu is displayed when entering the program, and also
 * any time '?' is entered. The menu includes storing/restoring
 * all information from the disk.
 * The search for a name is done first under the given parent. If that
 * fails, the whole tree is searched down from the root.
 * This program has only a primitive interface, its prime purpose
 * is to test the manipulation of the data.
 * Each table is simply one level of the tree, with the index derived
 * from the order of the branches.
 * Author: Jiri Soukup, Feb.18, 1989 */
#define ZZmain
#include <stdio.h>
#include "zzincl.h"
#define BSIZE 80
...
char *argv[];
{
    ...
    root=(Node *)NULL;
    ZZ_PLAIN_ALLOC(Node,1,root); /* root of the tree */
    root->width=1;
